...
const mp = MidcontractProtocol.buildByEnvironment("local", alice)
const random = (min, max) => Math.round(Math.random() * (max - min) + min);
const makeDepositId = () => random(100000, 1000000);

async function printBalance() {
  console.log('balance:', {
    platform: await mp.tokenBalance(platform.address),
    alice: await mp.tokenBalance(alice.address),
    bob: await mp.tokenBalance(bob.address),
  })
}
...
async function main() {
  ...
  const recipientData = await mp.escrowMakeDataHash(depositId.toString())
